        ...
        <template v-else-if="activeTab === 'analytics'">
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <input
                :value="config.netlifyWebhookUrl"
                @input="composition.setDashboardConfig({ netlifyWebhookUrl: ($event.target as HTMLInputElement).value })"
                placeholder="https://api.netlify.com/build_hooks/..."
                :class="inputClass"
                :style="inputStyle"
              />
              <p class="text-xs mt-1" :style="{ color: 'var(--theme-text-muted)' }">Used by Content Editors to trigger a site rebuild after publishing.</p>
            </div>
            <div>
