...
const variants = computed(() => registry.getVariantsBySection('footer'))
const selectedVariant = ref<ComponentVariant | null>(null)

const client = computed(() => workflow.clientInfo)
const currentYear = new Date().getFullYear()

const siteName = computed(() => client.value?.name ?? 'Your Business')

const copyrightText = computed(() =>
  `${currentYear} ${siteName.value}. All rights reserved.`
)
...
const iframeSrc = computed(() => {
  ...
  const params = new URLSearchParams({
    ...
    columns: JSON.stringify([
      ...
      { title: 'Legal', links: footerNavLinks.value }
    ]),
...
const selectForProject = (id: string) => {
  if (composition.composition.footer === id) {
    composition.setFooter(null)
  } else {
    composition.setFooter(id)
  }
}
...
<template>
  <div class="p-6 lg:p-8 max-w-7xl">
    ...
    <div class="mb-8">
      <h1 class="text-2xl font-bold" :style="{ color: 'var(--theme-text-primary)' }">Footer Variants</h1>
      <p class="mt-1" :style="{ color: 'var(--theme-text-secondary)' }">Browse and preview available footer components for client sites.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
      <div
        ...
        :class="composition.composition.footer === variant.id ? 'ring-2 shadow-md' : ''"
        :style="{
          backgroundColor: 'var(--theme-bg-card)',
          borderColor: composition.composition.footer === variant.id ? 'var(--theme-primary)' : 'var(--theme-border)',
      ...
      >
        <button @click="selectVariant(variant)" class="w-full p-5 text-left">
          <div class="flex items-start justify-between mb-1">
            ...
            <div
              v-if="composition.composition.footer === variant.id"
        ...
        <div class="px-5 pb-4">
          <button
            ...
            :style="composition.composition.footer === variant.id
          ...
          >
            {{ composition.composition.footer === variant.id ? 'Deselect' : 'Select for Project' }}
          </button>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        <button
          ...
          class="text-sm px-3 py-1 rounded-lg transition-colors shrink-0 ml-auto"
          :style="{ color: 'var(--theme-text-secondary)' }"
        >Close</button>
      </div>
